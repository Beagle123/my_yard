...
def generate_assets
  ...
  generate_class_and_method_list
  generate_just_file_list
  generate_frameset
end

def generate_just_file_list
  @file_list = true
  @items = options.files
  @list_title = "File List"
  @list_type = "file"
  @nav_url = "just_file_list.html"
  asset(url_for_list("just_file"), erb(:just_files))
  @file_list = nil
end

def generate_class_and_method_list
  @items = options.objects if options.objects
  @methods = prune_method_listing(Registry.all(:method), false)
  @methods = @methods.reject {|m| m.name.to_s =~ /=$/ && m.is_attribute? }
  @methods = @methods.sort_by {|m| m.name.to_s }
  asset(url_for_list("class_and_method"), erb(:class_and_method))
end
